...
"""
batch_eval_nmse.py
批量计算各 stage checkpoint 在测试集上的标准 NMSE。

标准 NMSE = Σ‖ŷ - y‖² / Σ‖y‖²   (在所有 masked patch 上累加)

用法:
    python batch_eval_nmse.py \
        --test_data_path /path/to/test.npy \
        --output_dir     ./output_R2P6 \
...
class FloatTensorDataset(torch.utils.data.Dataset):
    def __init__(self, npy_path: str, normalize: bool = False):
        ...
        assert data.ndim == 4 and data.shape[1] == 2, \
            f"Expected (N,2,H,W), got {data.shape}"
        if normalize:
...
def evaluate_nmse(model, data_loader, mask_ratio: float,
    ...
    分子分母均在 **全部样本的 masked patch** 上累加后再做除法，
    ...
    for samples, _ in data_loader:
        ...
        diff = pred - target                            # (B, L, C)

        # 在 patch 维度 C 上求平方和 → (B, L)
        se     = (diff   ** 2).sum(dim=-1)              # (B, L)
...
def find_stage_checkpoints(output_dir: str):
    ...
    for sd in stage_dirs:
        ...
        # checkpoint 文件名形如 checkpoint-199.pth → 取 epoch 数最大的
        def epoch_num(p):
...
# ──────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser('Batch NMSE evaluation')
    ...
    parser.add_argument('--normalize',   action='store_true',
                        help='与训练时一致：是否对测试数据做 z-score 归一化')
    ...
    print(f"Test samples: {len(dataset_test)}, shape: {dataset_test.data.shape}")
    ...
    for stage_idx, mask_ratio, ckpt_path in stages:
        ...
        # 每次重新构建模型（防止权重污染）
        model = models_mae_fas_GNN.__dict__[args.model](
        ...
        print(f"  NMSE = {nmse:.6e}   ({10*np.log10(nmse):.2f} dB)")
