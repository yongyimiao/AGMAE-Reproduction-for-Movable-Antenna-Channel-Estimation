...
# from util.pos_array_response_embed import get_2d_array_response_pos_embed
...
def build_knn_edge_index(num_nodes: int, grid_size: int, k: int, device: torch.device):
    """
    Build a static k-nearest-neighbor edge index for a square grid of
    (grid_size × grid_size) nodes, based on 2-D Euclidean distance.
    Returns edge_index: (2, E)  [source, target] — same for every sample
    in a batch (no batch dimension here; caller broadcasts with offset).
    """
    # node id → (row, col)
    rows = torch.div(torch.arange(num_nodes, device=device), grid_size, rounding_mode='floor')
    ...
    # for each node, pick k nearest neighbours (excluding self)
    dist.fill_diagonal_(float('inf'))
    _, nn_idx = dist.topk(k, dim=1, largest=False)             # (N, k)

    src = torch.arange(num_nodes, device=device).unsqueeze(1).expand_as(nn_idx).reshape(-1)
...
# ---------------------------------------------------------------------------
# GAT layer (paper eq. 11) — pure PyTorch, no DGL dependency
# ---------------------------------------------------------------------------

class GATLayer(nn.Module):
    """
    Single-head Graph Attention layer following AGMAE (eq. 11).
    Input/output feature dim: F_dim.
    """
    ...
    def forward(self, h: torch.Tensor, edge_index: torch.Tensor) -> torch.Tensor:
        ...
        # scoring: e(h_i, h_j) = a · LReLU(We · [h_i || h_j])
        e = self.act(self.We(torch.cat([h_dst, h_src], dim=-1))).squeeze(-1)  # (B, E)

        # softmax per destination node
        alpha = torch.full((B, N, N), float('-inf'), device=h.device, dtype=e.dtype)
        alpha[:, dst, src] = e
        alpha = F.softmax(alpha, dim=-1)           # (B, N, N)  — rows are dst
        alpha = torch.nan_to_num(alpha, nan=0.0)

        # aggregate: h'_i = σ( Σ_j α_ij · Wa · h_j )
...
class GATBlock(nn.Module):
    """
    One AGMAE decoder block: GAT + residual + LayerNorm + FFN (eq. 10).
    Mirrors the structure of timm's Transformer Block so it is a drop-in
    replacement inside the decoder.
    """
...
class MaskedAutoencoderViT(nn.Module):
    def __init__(self, img_size=20, patch_size=1, in_chans=2,
        ...
        self.grid_size    = self.patch_embed.grid_size   # needed for GAT
        ...
        # ---- decoder: mid-mapper + GAT blocks + pred head ----------------
        ...
        # Replace Transformer decoder blocks with GAT blocks
        ...
        # ---- GAT topology (fixed, built lazily) --------------------------
        self.gat_k_neighbors = gat_k_neighbors
        self._edge_index_cache: dict = {}   # device → edge_index
    ...
    # ------------------------------------------------------------------
    # GAT edge index (cached per device)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # patchify / unpatchify / masking (unchanged)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Decoder  — only the inner loop changes: GAT instead of ViT blocks
    # ------------------------------------------------------------------
    def forward_decoder(self, x, ids_restore):
        # mid-mapper: dim reduction + fill mask tokens + add pos embed
        ...
        # GAT decoder blocks (paper eq. 10-11)
    ...
    def forward_loss_NMSE(self, imgs, pred, mask):
        target = self.patchify(imgs)                          # (B, 400, 2)
        mse    = ((pred - target) ** 2).mean(dim=-1)          # (B, 400)
        power  = (target ** 2).mean(dim=-1) + 1e-6
        nmse   = mse / power
        loss   = (nmse * mask).sum() / mask.sum()
        return loss
...
# ---------------------------------------------------------------------------
# Model factory functions (unchanged signatures)
# ---------------------------------------------------------------------------

def mae_fas_channel_model(**kwargs):
    return MaskedAutoencoderViT(
        img_size=20, patch_size=1, in_chans=2,
        embed_dim=512, depth=12, num_heads=4,
        decoder_embed_dim=128, decoder_depth=12, decoder_num_heads=4,
...
def mae_fas_channel_model_posv2(**kwargs):
    return MaskedAutoencoderViT(
        img_size=20, patch_size=1, in_chans=2,
        embed_dim=392,
        depth=8,
